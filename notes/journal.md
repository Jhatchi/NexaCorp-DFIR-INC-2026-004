...
5. Cracked the hash offline. The lab workstation had no john, no hashcat, no rockyou, and no internet (apt failed on DNS). Moved the crack to the analyst Mac: downloaded rockyou from the public source, ran john raw-md5, got `P@ssw0rd123` in under a second. Cracking stays off the live server by design.
...
- **Complexity is not strength.** `P@ssw0rd123` passes every complexity checkbox and still falls instantly. Best single talking point from this lab.
